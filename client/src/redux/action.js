...
export const getJobs=({page,pagesize})=>{
    return async(dispatch,getState,api)=>{
        try {
            const res=await fetch(`http://localhost:1200/getjobs/jobs?pagesize=${pagesize}&page=${page}`)
               const received=await res.json()
...
export const getCart=({page,pagesize,id})=>{
    return async(dispatch,getState,api)=>{
        try {
            const res=await fetch(`http://localhost:1200/getcart/carts/${id}?pagesize=${pagesize}&page=${page}`)
               const received=await res.json()
...
export const searched=({page,pagesize,text})=>{
    return async(dispatch,getState,api)=>{
        try {
            const res=await fetch(`http://localhost:1200/getjobs/jobs/${text}?pagesize=${pagesize}&page=${page}`)
               const received=await res.json()
